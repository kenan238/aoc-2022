...
def is_valid_move(cavern, rock, pos): # check for walls and if its possible to move at all
    return all(0 <= (c + pos).real <= 6 for c in rock) and all(not c + pos in cavern for c in rock)

def add_rock(cavern, rock, wind, wi):
    pos = 1j * (cavern_height(cavern) + 4) # get cavern height
    while True:
        h_move, wi = 1 if wind[wi % len(wind)] == '>' else -1, wi + 1 # move down
        if is_valid_move(cavern, rock, pos + h_move): # check if move is valid
            pos += h_move
        if is_valid_move(cavern, rock, pos - 1j): # check if move is valid, if it is, move down
            pos -= 1j
        else:
            cavern.update({c + pos for c in rock}) # update cavern with new rock positions
            return cavern, wi

def part1(rocks, wind):
    wi, cavern = 0, set({ x for x in range(7) }) # construct the cavern
    for i in range(2022):
        cavern, wi = add_rock(cavern, rocks[i % 5], wind, wi) # run ticks
    print(cavern_height(cavern)) # return height
